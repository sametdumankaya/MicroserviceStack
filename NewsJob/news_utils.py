...
class NewsUtils:
    ...
    def analyzeNewsWithAPI(self, symbol, todays_start_date_for_news, todays_end_date_for_news, page_size=10,
        ...
        if (not self.redis_output_client.ping()):
            print("Unable to ping output redis")
            return
        ...
        _ = self.redis_output_client.lpush("NewsIndustry", pickle.dumps(dict_news))
        ...
        _ = self.redis_output_client.lpush("NewsCompany", pickle.dumps(dict_news))
    ...
    def getNewsFromRedisBetweenDates(self, startDate, endDate):
        dt = datetime.datetime.strptime(startDate, '%Y-%m-%d %H:%M:%S.%f')
        startTime = dt.strftime('%s') + '000'
        dt = datetime.datetime.strptime(endDate, '%Y-%m-%d %H:%M:%S.%f')
        endTime = dt.strftime('%s') + '000'
        dailyNews = {}
        topNews = {}
        try:
            news = self.redis_output_client.xrange("NewsSentimentResult", f'{startTime}', f'{endTime}')
